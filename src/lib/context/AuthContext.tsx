...
//      */
//     // Inside AuthContext.tsx -> onAuthStateChange
...
//       if (event === 'SIGNED_IN' && authPages.includes(window.location.pathname)) {
//         console.log("[AUTH] User signed in on auth page, redirecting to dashboard...");
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // --- UPDATED VALUE TO INCLUDE forgotPassword ---
  const value = useMemo(() => ({ 
    user, 
    loading, 
    login, 
    ...
    signInWithGoogle,
    logout,
    forgotPassword // <--- CRITICAL: Pass it here!
  }), [user, loading]);
